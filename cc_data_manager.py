...
if __name__ == '__main__':
    ...
    p0 = run(['globus', 'endpoint', 'local-id'],capture_output=True)
    if(p0.returncode!=0):
        print("\n")
    ...
    p1 = run(['globus', 'ls', local_endpoint], capture_output=True )
    ...
    p2 = run( [ 'globus', 'ls', remote_path], capture_output=True )
    ...
    if(p2_exit_code == "c1"):
        p3 = run( [ 'globus', 'mkdir', remote_path], capture_output=True )
        if(p3.returncode) == 0:
    ...
    p4 = run( [ 'globus', 'transfer', '--notify', 'failed,inactive,succeeded', '--recursive', '--sync-level', args.sync_level, local_path, remote_path], capture_output=True )
    if(p4.returncode!=0):
        print("\n")
    ...
    if(len(email_list_all) > 0):
        for tmp_email in email_list_all:            
            ...
            p5= run(['globus', 'endpoint', 'permission', 'create', '--provision-identity', tmp_email, '--permissions', 'r', remote_path, '--notify-email', tmp_email], capture_output=True )
    ...
    if(len(group_list_all) > 0):
        for tmp_group in group_list_all:  
            ...
            p6= run(['globus', 'endpoint', 'permission', 'create', '--group', tmp_group, '--permissions', 'r', remote_path], capture_output=True)
